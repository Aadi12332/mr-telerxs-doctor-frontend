...
export default function NoteModal({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      {step === "list" && (
        <div className="bg-[#B2E5FD] rounded-[20px] max-w-[998px]  max-h-[95vh] overflow-auto w-full p-8">
          ...
          <div className="flex justify-end mt-6">
            <button
              onClick={onClose}
              className="border border-[#0000004D] text-[20px] px-12 rounded-[10px] h-[50px]"
            >
              Cancel
            </button>
          </div>
        ...
        <div className="bg-[#B2E5FD] rounded-[20px] max-w-[998px] w-full p-8">
          <div className="flex items-center gap-4 mb-4">
            <button onClick={() => setStep("list")}>
              <img src={backarrow} className="w-6" />
            </button>
            <p className="text-[22px] font-medium">
              Medical Accommodation Note
            </p>
            <span className="ml-auto text-[#00000080]">
              2025-12-24 10:15 AM
            </span>
          </div>
